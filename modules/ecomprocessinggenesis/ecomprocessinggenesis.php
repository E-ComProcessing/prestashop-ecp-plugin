...
class EComProcessingGenesis extends PaymentModule
{
	public function __construct()
	{
		$this->name         = 'ecomprocessinggenesis';
		$this->displayName  = 'E-ComProcessing Genesis';
		...
		if (!isset($this->config['ECOMPROCESSING_GENESIS_USERNAME']) ||
		    !isset($this->config['ECOMPROCESSING_GENESIS_PASSWORD']) ||
		    !isset($this->config['ECOMPROCESSING_GENESIS_TOKEN'])) {
			$this->warning = $this->l('You need to set your credentials (username, password, token), in order to access our service!');
		}

		$this->use3D = ($this->config['ECOMPROCESSING_GENESIS_USE3D'] == 'true') ? true : false;
		...
		if (Tools::isSubmit('process_refund') && isset($_POST['refund_amount']) && isset($_POST['id_transaction'])) {
			...
			try {
				...
				if (class_exists('Logger')) {
					$message = $this->l( 'E-ComProcessing Genesis - An error has occurred, while trying to issue a refund!' );
	...
	private function prepareData()
	{
		...
		$expiration = array_combine(  array( 'month', 'year' ),  explode( '/', Tools::getValue('ecomprocessinggenesis-expiry') ) );
		...
		$transactionData->card_number       = str_replace(' ', '', Tools::getValue('ecomprocessinggenesis-number'));
		$transactionData->card_holder       = Tools::getValue('ecomprocessinggenesis-name');
		$transactionData->cvv               = Tools::getValue('ecomprocessinggenesis-cvc');
	...
	function processPayment()
	{
		try {
			...
			if (class_exists('Logger')) {
				$message = $this->l( 'E-ComProcessing Genesis - Error processing transaction!' );
	...
	private function initGenesis()
	{
		...
		\Genesis\GenesisConfig::setUsername( $this->config['ECOMPROCESSING_GENESIS_USERNAME'] );
		\Genesis\GenesisConfig::setPassword( $this->config['ECOMPROCESSING_GENESIS_PASSWORD'] );
		\Genesis\GenesisConfig::setToken( $this->config['ECOMPROCESSING_GENESIS_TOKEN'] );

		switch ($this->config['ECOMPROCESSING_GENESIS_ENVIRONMENT']) {
			default:
			case 'sandbox':
			...
			'form' => array(
				...
				'input' => array(
					array(
						...
						'name'      => 'ECOMPROCESSING_GENESIS_USERNAME',
						'size'      => 20,
						'required'  => true
					),
					array(
						...
						'name'      => 'ECOMPROCESSING_GENESIS_PASSWORD',
						'size'      => 20,
						'required'  => true
					),
					array(
						'type'      => 'text',
						'label'     => $this->l('Token'),
						'name'      => 'ECOMPROCESSING_GENESIS_TOKEN',
						'size'      => 20,
						'required'  => true
					),
					array(
						...
						'name'      => 'ECOMPROCESSING_GENESIS_ENVIRONMENT',
						'options'   => array(
							'query' => array(
								array(
									'id'    => 'sandbox',
					...
					array(
						...
						'name'      => 'ECOMPROCESSING_GENESIS_USE3D',
						'options'   => array(
							'query' => array(
								array(
									'id'    => 'false',
	...
	{
		return array(
			'ECOMPROCESSING_GENESIS_USERNAME',
			'ECOMPROCESSING_GENESIS_PASSWORD',
			'ECOMPROCESSING_GENESIS_TOKEN',
			'ECOMPROCESSING_GENESIS_ENVIRONMENT',
			'ECOMPROCESSING_GENESIS_USE3D',
		);
	}
