...
class EComProcessingNotificationModuleFrontController extends ModuleFrontController
{
	...
	private function bootstrapGenesis()
	{
		...
		\Genesis\GenesisConfig::setUsername(
			Configuration::get('ECOMPROCESSING_GENESIS_USERNAME')
		);
		\Genesis\GenesisConfig::setPassword(
			Configuration::get('ECOMPROCESSING_GENESIS_PASSWORD')
		);
		\Genesis\GenesisConfig::setToken(
			Configuration::get('ECOMPROCESSING_GENESIS_TOKEN')
		);

		switch (Configuration::get('ECOMPROCESSING_GENESIS_ENVIRONMENT')) {
			default:
			case 'sandbox':
