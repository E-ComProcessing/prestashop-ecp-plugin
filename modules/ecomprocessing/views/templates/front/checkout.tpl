...
{if version_compare($ecomprocessing['presta']['version'], '1.5', '>=') && version_compare($ecomprocessing['presta']['version'], '1.6', '<') }
    ...
    {capture name=path}
        <a href="{$ecomprocessing['checkout']['links']['back']|escape:'html':'UTF-8'}" title="{l s='Go back to the Checkout' mod='ecomprocessing'}">
    ...
    {else}
        ...
        {if $ecomprocessing['checkout']['error']}
            <div class="row row-spacer">
                <div class="alert alert-warning alert-dismissable error-wrapper" style="width:75%;margin:15px auto;">
                    ...
                    {$ecomprocessing['checkout']['error']|escape:html:'UTF-8'}
                </div>
            </div>
        {/if}
        ...
        <form action="{$ecomprocessing['checkout']['links']['confirm']|escape:'html'}" method="POST" name="">
            ...
            <p class="cart_navigation" id="cart_navigation">
                ...
                <a href="{$ecomprocessing['checkout']['links']['back']|escape:'html':'UTF-8'}" class="button_large">{l s='Other payment methods' mod='ecomprocessing'}</a>
            </p>
        </form>
    {/if}
...
{if version_compare($ecomprocessing['presta']['version'], '1.6', '>=') && version_compare($ecomprocessing['presta']['version'], '1.7', '<') }
    ...
    {/if}
        ...
        <form action="{$ecomprocessing['checkout']['links']['confirm']|escape:'html'}" method="post">
            <input type="hidden" name="checkout" value="confirmed" />
            <input type="hidden" name="submit{$ecomprocessing['name']['module']}Checkout" value="true" />
            ...
            <p class="cart_navigation clearfix" id="cart_navigation">
                <a href="{$ecomprocessing['checkout']['links']['back']|escape:'html':'UTF-8'}" class="button-exclusive btn btn-default">
