...
class Ecomprocessing extends PaymentModule
{
     ...
     */
    public const SETTING_ECOMPROCESSING_USERNAME = 'ECOMPROCESSING_USERNAME';
    public const SETTING_ECOMPROCESSING_PASSWORD = 'ECOMPROCESSING_PASSWORD';
    public const SETTING_ECOMPROCESSING_ENVIRONMENT = 'ECOMPROCESSING_ENVIRONMENT';
    public const SETTING_ECOMPROCESSING_CHECKOUT = 'ECOMPROCESSING_CHECKOUT';
    public const SETTING_ECOMPROCESSING_CHECKOUT_TRX_TYPES = 'ECOMPROCESSING_CHECKOUT_TRX_TYPES';
    public const SETTING_ECOMPROCESSING_ALLOW_PARTIAL_CAPTURE = 'ECOMPROCESSING_ALLOW_PARTIAL_CAPTURE';
    public const SETTING_ECOMPROCESSING_ALLOW_PARTIAL_REFUND = 'ECOMPROCESSING_ALLOW_PARTIAL_REFUND';
    public const SETTING_ECOMPROCESSING_ALLOW_VOID = 'ECOMPROCESSING_ALLOW_VOID';
    public const SETTING_ECOMPROCESSING_ADD_JQUERY_CHECKOUT = 'ECOMPROCESSING_ADD_JQUERY_CHECKOUT';
    public const SETTING_ECOMPROCESSING_WPF_TOKENIZATION = 'ECOMPROCESSING_WPF_TOKENIZATION';
    public const SETTING_ECOMPROCESSING_CHECKOUT_BANK_CODES = 'ECOMPROCESSING_CHECKOUT_BANK_CODES';
    public const SETTING_ECOMPROCESSING_THREEDS_ALLOWED = 'ECOMPROCESSING_THREEDS_ALLOWED';
    ...
    {
        return $this->getBoolConfigurationValue(self::SETTING_ECOMPROCESSING_CHECKOUT);
    }
    ...
    {
        return $this->getBoolConfigurationValue(self::SETTING_ECOMPROCESSING_WPF_TOKENIZATION);
    }
    ...
    public function populateTransactionData()
    {
        ...
        $data->threeds_challenge_indicator = Configuration::get(
            self::SETTING_ECOMPROCESSING_THREEDS_CHALLENGE_INDICATOR
        );
        ...
        $data->threeds_shipping_indicator = EcomprocessingThreeds::getShippingIndicator(
            $cart,
            $invoice,
            $shipping,
            Cart::isGuestCartByCartId($cart->id)
        );
        $data->threeds_reorder_items_indicator = EcomprocessingThreeds::getReorderItemsIndicator(
            $cart,
            $customer,
            Cart::isGuestCartByCartId($cart->id)
        );
        ...
        if (!Cart::isGuestCartByCartId($cart->id)) {
            ...
            $data->threeds_registration_date = EcomprocessingThreeds::findFirstCustomerOrderDate(
                $customer
            );
            $data->threeds_registration_indicator = EcomprocessingThreeds::getRegistrationIndicator($customer);
            $data->threeds_last_change_date = EcomprocessingThreeds::findLastChangeDate(
                $customer,
                $cart->id_lang
            );
            $data->threeds_update_indicator = EcomprocessingThreeds::getUpdateIndicator(
                $customer,
                $cart->id_lang
            );
            $data->threeds_password_change_date = $customer->last_passwd_gen;
            $data->threeds_password_change_indicator = EcomprocessingThreeds::getPasswordIndicator($customer);

            $shippingAddressDateFirstUsed = EcomprocessingThreeds::findShippingAddressDateFirstUsed(
                $customer,
                $cart
            );
            $data->threeds_shipping_address_date_first_used = $shippingAddressDateFirstUsed;
            $data->threeds_shipping_address_usage_indicator = EcomprocessingThreeds::getShippingAddressUsageIndicator(
                $shippingAddressDateFirstUsed
            );
            $data->transactions_activity_last_24_hours = EcomprocessingThreeds::findNumberOfOrdersForAPeriod(
                $customer->id,
                EcomprocessingThreeds::ACTIVITY_24_HOURS
            );
            $data->transactions_activity_previous_year = EcomprocessingThreeds::findNumberOfOrdersForAPeriod(
                $customer->id,
                EcomprocessingThreeds::ACTIVITY_1_YEAR
            );
            $data->purchases_count_last_6_months = EcomprocessingThreeds::findNumberOfOrdersForLastSixMonths(
                $customer->id
            );
        }

        // SCA Exemption
        $data->sca_exemption_value = Configuration::get(self::SETTING_ECOMPROCESSING_SCA_EXEMPTION);
        $data->sca_exemption_amount = Configuration::get(self::SETTING_ECOMPROCESSING_SCA_EXEMPTION_AMOUNT);
    ...
    public function getConfigKeys()
    {
        return [
            ...
            self::SETTING_ECOMPROCESSING_THREEDS_ALLOWED,
            self::SETTING_ECOMPROCESSING_THREEDS_CHALLENGE_INDICATOR,
            self::SETTING_ECOMPROCESSING_SCA_EXEMPTION,
            self::SETTING_ECOMPROCESSING_SCA_EXEMPTION_AMOUNT,
        ];
    }
    ...
    private function getFormCredentialsFields()
    {
        return [
            [
                ...
                'name' => self::SETTING_ECOMPROCESSING_USERNAME,
                'size' => 20,
                'required' => true,
            ],
            [
                ...
                'name' => self::SETTING_ECOMPROCESSING_PASSWORD,
                'size' => 20,
                'required' => true,
            ],
            [
    ...
    private function getFormTransactionFields()
    {
        return [
            ...
            [
                ...
                'id' => self::SETTING_ECOMPROCESSING_CHECKOUT_TRX_TYPES,
                'name' => self::SETTING_ECOMPROCESSING_CHECKOUT_TRX_TYPES . '[]',
                'multiple' => true,
                'options' => [
    ...
    private function getFormThreedsFields()
    {
        return [
            [
                ...
                'name' => self::SETTING_ECOMPROCESSING_THREEDS_ALLOWED,
                'values' => [
                    [
                        'id' => 'active_on',
                        'value' => '1',
            ...
            [
                ...
                'id' => self::SETTING_ECOMPROCESSING_THREEDS_CHALLENGE_INDICATOR,
                'name' => self::SETTING_ECOMPROCESSING_THREEDS_CHALLENGE_INDICATOR,
                'multiple' => false,
                'options' => [
                    'query' => $this->generateOptionsFromArray(EcomprocessingThreeds::getChallengeIndicators()),
                    'id' => 'id',
                    'name' => 'name',
                ],
            ],
    ...
    private function getFormScaFields()
    {
        return [
            [
                ...
                'id' => self::SETTING_ECOMPROCESSING_SCA_EXEMPTION,
                'name' => self::SETTING_ECOMPROCESSING_SCA_EXEMPTION,
                'multiple' => false,
                'options' => [
            ...
            [
                ...
                'name' => self::SETTING_ECOMPROCESSING_SCA_EXEMPTION_AMOUNT,
                'required' => true,
            ],
        ];
    }
    ...
    private function init()
    {
        ...
        /* ecomprocessing Threeds helper */
        include_once dirname(__FILE__) . '/classes/EcomprocessingThreeds.php';
    ...
    public function applyGenesisConfig()
    {
        ...
        \Genesis\Config::setUsername(
            Configuration::get('ECOMPROCESSING_USERNAME')
        );
        \Genesis\Config::setPassword(
            Configuration::get('ECOMPROCESSING_PASSWORD')
        );

        \Genesis\Config::setEnvironment(
            Configuration::get('ECOMPROCESSING_ENVIRONMENT')
        );
    }
    ...
    {
        $defaultConfigItems = [
            self::SETTING_ECOMPROCESSING_CHECKOUT => '0',
            self::SETTING_ECOMPROCESSING_CHECKOUT_TRX_TYPES => [
                Genesis\API\Constants\Transaction\Types::AUTHORIZE,
                Genesis\API\Constants\Transaction\Types::SALE,
            ],
            self::SETTING_ECOMPROCESSING_ALLOW_PARTIAL_CAPTURE => '1',
